...
To quit the game, type q`;
    ...
    this.makeStep(question.options, +answer);
  },

  makeStep(currentOptions, userAnswer) {
    const chosenOptions = currentOptions[userAnswer - 1];
    this.log(chosenOptions.text);
    if (chosenOptions.nextText !== -1) {
      const nextQuestion = this.textNodes.find((q) => q.id === chosenOptions.nextText);
